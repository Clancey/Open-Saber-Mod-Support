...
Gameplay objects and the default environment are built to match how Beat Saber plays and reads:

* **Movement** (`game/scripts/NoteMovementData.gd`): notes, bombs, chains, arcs and walls follow the standard
  beatmap motion model. Objects travel at the map's note jump speed,
...
* **Notes/bombs/walls/sabers**: generated meshes (`game/assets/beatsaber/meshes`, built by
  `tools/art/gen_meshes.py`) with shaders for the note body, bomb, obstacle core/frame, saber blade and slash
  trail (colors, rim dimming, cut edge glow, 0.4 s trail with a white section, 1 m blade).
* **Environment** (`game/event_driver.tscn`: rings, pillars, buildings,
...
* **Other environments**: `game/environments/<EnvironmentName>.tscn` is loaded when a map's Info.dat names it
  (`_environmentName` / `environmentNames`), falling back to "The First". Each scene is assembled from generated
  meshes (neon tubes grouped by light id, rotating laser pivots,
...
* **Art**: every image, mesh and font page in the project is generated from scratch by the scripts in `tools/art`
  — `gen_menu_art.py` (menu tiles, icons and the neon OPEN SABER sign with its flickering letter and scattered
  note cubes), `gen_textures.py` (gameplay glow and noise textures), `gen_meshes.py` (all gameplay and
  environment meshes) and `gen_fonts.py` (the Teko bitmap atlases). No art is copied from any other game.
* Default colors are Beat Saber's "The First" scheme. Chain links use a generated slice mesh, note cuts spawn
  sparkle/explosion bursts, bombs explode, and the environment's directional lights follow the light
  events with per-light weights.
* Menus use a Beat Saber style theme (`game/ui/beat_saber_theme.tres`: the Teko font (OFL) rasterised by
  `tools/art/gen_fonts.py`, dark rounded panels, cyan highlights, cyan Play button), the Solo / Online / Campaign / Party tiles,
  the level list rows (cover, song, author, BPM) with the cover / characteristic / difficulty / details column on
  the right, and the pause and "LEVEL CLEARED" results panels. The HUD has the combo panel
