...
* **Art**: the menu tiles, icons, the neon OPEN SABER sign over the menu (with its flickering letter and the note
  cubes scattered around, like the original's menu environment) and the gameplay glow/noise textures are generated by
  `tools/art/gen_menu_art.py` and `tools/art/gen_textures.py`, so no art is copied from any other game.
